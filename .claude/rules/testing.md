...
- e2e target (CLAUDE.md "Foundations status"): `backend/test/*.e2e-spec.ts` with supertest against the real
  `AppModule` and disposable Postgres/Redis containers. Cover the auth flow, envelope and error format, validation,
  and pagination.
